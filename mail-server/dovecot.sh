...
function _setup_dovecot_quota() {
  ...
  if [[ ${ACCOUNT_PROVISIONER} != 'FILE' ]] || [[ ${SMTP_ONLY} -eq 1 ]] || [[ ${ENABLE_QUOTAS} -eq 0 ]]; then
    # disable dovecot quota in dovecot confs
...
function _setup_dovecot_local_user() {
  ...
  # Create a mail account for POSTMASTER_ADDRESS with a random password
  if [[ -n "${POSTMASTER_ADDRESS}" ]]; then
    _log 'trace' "Creating mail account for ${POSTMASTER_ADDRESS}"
    
    # Generate a random 16-character password
    local RANDOM_PASSWORD
    RANDOM_PASSWORD=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 16)
    
    # Hash the password using doveadm pw (SHA512-CRYPT scheme)
    local PASSWORD_HASH
    PASSWORD_HASH=$(doveadm pw -s SHA512-CRYPT -p "${RANDOM_PASSWORD}")
    
    # Ensure the postfix-accounts.cf file exists
    ...
    if ! grep -q "^${POSTMASTER_ADDRESS}|" /tmp/docker-mailserver/postfix-accounts.cf; then
      echo "${POSTMASTER_ADDRESS}|${PASSWORD_HASH}" >> /tmp/docker-mailserver/postfix-accounts.cf
      _log 'trace' "Account ${POSTMASTER_ADDRESS} created with random password"
    else
      _log 'trace' "Account ${POSTMASTER_ADDRESS} already exists, skipping creation"
    fi
  else
    _log 'warn' "POSTMASTER_ADDRESS is not set, cannot create default account"
  fi
